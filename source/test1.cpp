...
// ==========================================
template <typename T>
void readAndConvert(std::istream& is, std::vector<float>& outData, size_t count, bool isCompressed) {
	std::vector<T> buffer(count);
	if (isCompressed) {
		// --- zlib解凍ロジック (提供コードのロジックをT型用に適用) ---
		// ファイル全体を読み込み
		is.seekg(0, std::ios::end);
		size_t fileSize = is.tellg();
		is.seekg(0, std::ios::beg);
		std::vector<unsigned char> compressed(fileSize);
		is.read((char*)compressed.data(), fileSize);

		z_stream strm = {};
		strm.next_in = compressed.data();
		strm.avail_in = (uInt)fileSize;
		strm.next_out = (Bytef*)buffer.data();
		strm.avail_out = (uInt)(count * sizeof(T));
		...
		is.read((char*)buffer.data(), count * sizeof(T));
	}

	// 0.0 - 1.0 への正規化
	T minV = buffer[0], maxV = buffer[0];
	for (T v : buffer) {
		if (v < minV) minV = v;
		if (v > maxV) maxV = v;
	}
	...
	for (size_t i = 0; i < count; ++i) {
		outData[i] = (float)(buffer[i] - minV) / range;
	}
}
...
Image3D loadFromMHD(const std::string& mhdPath) {
	...
	std::string type = meta["ElementType"];
	...
	// 型に応じた分岐
	if (type == "MET_SHORT") {
		readAndConvert<short>(rfs, vol.data, voxelCount, isCompressed);
	}
	else if (type == "MET_INT") {
		readAndConvert<int>(rfs, vol.data, voxelCount, isCompressed);
	}
	else if (type == "MET_DOUBLE") {
		readAndConvert<double>(rfs, vol.data, voxelCount, isCompressed);
	}
	else if (type == "MET_FLOAT") {
		readAndConvert<float>(rfs, vol.data, voxelCount, isCompressed);
	}
	else if (type == "MET_UCHAR") {
		readAndConvert<unsigned char>(rfs, vol.data, voxelCount, isCompressed);
	}
	else {
		throw std::runtime_error("Unsupported ElementType: " + type);
	}
...
int main() {
	try {
		...
		Image3D fixed = loadFromMHD("2 CT Images.mhd");
		Image3D moving = loadFromMHD("PET_BODY_CTAC.mhd");
